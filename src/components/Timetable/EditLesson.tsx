...
import { updateLesson, deleteLesson, Lesson } from '../../services/lessonService';
...
export default function EditLesson({ isOpen, onClose, onLessonUpdated, lesson }: EditLessonProps) {
  ...
  useEffect(() => {
    if (isOpen && lesson) {
      setFormData({
        ...
        repeat: lesson.repeat || 'never',
        sourceDate: lesson.sourceDate,
      });
      setStartTime(lesson.time || "08:00 AM");
    }
  }, [isOpen, lesson]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await updateLesson(lesson._id, formData as Lesson);
      onLessonUpdated();
      onClose();
    } catch (error) {
      console.error(error);
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4 font-sans">
      <div className="bg-[#FFFCF8] rounded-3xl w-full max-w-sm overflow-hidden shadow-xl relative flex flex-col max-h-[90vh]">
        ...
        <div className="flex justify-between items-center p-6 pb-2">
          <h2 className="text-xl text-gray-800 font-sister ">Edit Lesson</h2>
          <div className="flex items-center gap-2">
            <button type="button" onClick={handleDelete} disabled={deleting} className="text-rose-500 hover:bg-rose-100 p-2 rounded-full transition-colors">
              <Trash2 size={20} />
            </button>
        ...
        <div className="overflow-y-auto px-6 pb-6 custom-scrollbar">
          <form id="edit-lesson-form" onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div className="mt-4 pb-2 ">
              <button 
                ...
                disabled={loading || !formData.unitName?.trim() || deleting}
              >
                {loading ? (
                  <>
